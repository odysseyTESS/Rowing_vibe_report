import streamlit as st
import google.generativeai as genai
import requests
from datetime import datetime

# --- 初期設定 ---
st.set_page_config(page_title="Boat-Vibe-Relayer", layout="centered", page_icon="🚣‍♂️")
st.title("🚣‍♂️ Boat-Vibe Relayer")
st.caption("最新のGemini 2.5で、ボート部の報告を自動化します。")

# Secretsの取得
GEMINI_API_KEY = st.secrets["GEMINI_API_KEY"]
SLACK_WEBHOOK_URL = st.secrets["SLACK_WEBHOOK_URL"]

# --- Geminiのセットアップ ---
genai.configure(api_key=GEMINI_API_KEY)

# あなたが掘り当てた「正解のモデル名」を指定します
model = genai.GenerativeModel('gemini-2.5-flash')

# --- UI部分 ---
uploaded_file = st.file_uploader("音声ファイル (m4a) をアップロード", type=["m4a"])

if uploaded_file is not None:
    st.audio(uploaded_file)
    
    if st.button("報告書を生成＆Slackに送信"):
        with st.spinner("最新のAIが解析中..."):
            try:
                # 1. 音声データの読み込み
                audio_data = uploaded_file.read()
                
                # 今日の日付を取得
                date_str = datetime.now().strftime("%Y/%m/%d")
                
                # 2. 魂のプロンプト（完全クリーン版）
                prompt = f"""
あなたはボート部の優秀なマネージャーです。
選手の練習後の独り言（音声）を聞き取り、指定のフォーマットに正確に落とし込んでください。

【ルール】
1. 日付は今日の日付（{date_str}）と曜日を記載してください。
2. 音声から『メニュー』『目標』『結果』を抽出してください。
3. 『振り返り』はKPT形式（Keep: 良かった点、Problem: 課題、Try: 次にやること）で整理。
4. ボート用語（UT, B1, B2, RPE, エルゴ, 艇庫など）を正しく変換してください。
5. 最後に『（さらに何かあれば）』として、雑談やエピソードを詳しく記載してください。
※署名や特定の住所情報は一切不要です。

【出力フォーマット】
日付 [日付]
【メニュー】
[内容]

【目標】
[内容]

【結果】
[内容]

【振り返り】
K:
P:
T:

（さらに何かあれば）
[内容]
"""

                # 3. Gemini 2.5 による解析
                response = model.generate_content([
                    prompt,
                    {"mime_type": "audio/m4a", "data": audio_data}
                ])
                
                report_text = response.text
                
                # 4. Slackへ送信
                slack_payload = {"text": report_text}
                response_slack = requests.post(SLACK_WEBHOOK_URL, json=slack_payload)
                
                if response_slack.status_code == 200:
                    st.success("✅ ついに成功！Slackへの投稿が完了しました！")
                    st.balloons()
                    st.text_area("生成内容:", value=report_text, height=300)
                else:
                    st.error(f"Slack送信失敗: {response_slack.status_code}")
                
            except Exception as e:
                st.error(f"解析エラーが発生しました: {e}")
